...
void ArduboyLowMem::start() // deprecated
{
  begin();
}

void ArduboyLowMem::begin()
{
  boot(); // required
...
void ArduboyLowMem::setFrameRate(uint8_t rate)
{
  frameRate = rate;
  eachFrameMillis = 1000/rate;
}

bool ArduboyLowMem::everyXFrames(uint8_t frames)
{
  return frameCount % frames == 0;
}

bool ArduboyLowMem::nextFrame()
{
  long now = millis();
  uint8_t remaining;
...
int ArduboyLowMem::cpuLoad()
{
  return lastFrameDurationMs*100 / eachFrameMillis;
}

void ArduboyLowMem::initRandomSeed()
{
...
uint16_t ArduboyLowMem::rawADC(byte adc_bits)
{
  ADMUX = adc_bits;
...
void ArduboyLowMem::clearDisplay() // deprecated
{
  clear();
}

void ArduboyLowMem::clear()
{
  fillScreen(BLACK);
}

void ArduboyLowMem::drawPixel(int x, int y, uint8_t color)
{
...
uint8_t ArduboyLowMem::getPixel(uint8_t x, uint8_t y)
{
  uint8_t row = y / 8;
...
void ArduboyLowMem::drawCircle(int16_t x0, int16_t y0, uint8_t r, uint8_t color)
{
  int16_t f = 1 - r;
  int16_t ddF_x = 1;
  int16_t ddF_y = -2 * r;
...
void ArduboyLowMem::drawCircleHelper
...
void ArduboyLowMem::fillCircle(int16_t x0, int16_t y0, uint8_t r, uint8_t color)
{
  drawFastVLine(x0, y0-r, 2*r+1, color);
  fillCircleHelper(x0, y0, r, 3, 0, color);
}

void ArduboyLowMem::fillCircleHelper
...
void ArduboyLowMem::drawFastVLine
...
void ArduboyLowMem::drawFastHLine
...
void ArduboyLowMem::fillScreen(uint8_t color)
{
  // C version : 
  //
...
void ArduboyLowMem::drawRoundRect
...
void ArduboyLowMem::fillRoundRect
(int16_t x, int16_t y, uint8_t w, uint8_t h, uint8_t r, uint8_t color)
{
  // smarter version
...
void ArduboyLowMem::drawTriangle
...
void ArduboyLowMem::fillTriangle
(int16_t x0, int16_t y0, int16_t x1, int16_t y1, int16_t x2, int16_t y2, uint8_t color)
{
...
/*        int sBufferIndex;
          uint8_t data;
          if ( bRow >= 0 ) {
            sBufferIndex = (bRow*WIDTH) + x + iCol;
            data = pgm_read_byte(bitmap+(a*w)+iCol) << yOffset;
          }
          if (yOffset && bRow<(HEIGHT/8)-1 && bRow > -2) {
            sBufferIndex = ((bRow+1)*WIDTH) + x + iCol;
            data = pgm_read_byte(bitmap+(a*w)+iCol) >> (8-yOffset);
          }
          if      ( color == WHITE )  this->sBuffer[ sBufferIndex ] |= data;
          else if ( color == BLACK )  this->sBuffer[ sBufferIndex ] &= ~(data);
          else                        this->sBuffer[ sBufferIndex ] ^= data; */
          if (bRow >= 0) {
...
void ArduboyLowMem::drawSlowXYBitmap
...
void ArduboyLowMem::setCursor(int16_t x, int16_t y)
{
  cursor_x = x;
  cursor_y = y;
}

void ArduboyLowMem::setTextSize(uint8_t s)
{
  // textsize must always be 1 or higher
  textsize = max(1,s); 
}

void ArduboyLowMem::setTextWrap(boolean w)
{
  wrap = w;
}

size_t ArduboyLowMem::write(uint8_t c)
{
  if (c == '\n')
  {
    cursor_y += textsize*8;
...
void ArduboyLowMem::display()
{
  this->paintScreen(sBuffer);
}

unsigned char* ArduboyLowMem::getBuffer()
{
  return sBuffer;
}


boolean ArduboyLowMem::pressed(uint8_t buttons)
{
  return (buttonsState() & buttons) == buttons;
}

boolean ArduboyLowMem::notPressed(uint8_t buttons)
{
  return (buttonsState() & buttons) == 0;
}

void ArduboyLowMem::swap(int16_t& a, int16_t& b)
{
  int temp = a;
  a = b;
  b = temp;
